...
public:
	...
	AdjustableCamera()
	{
		//By default look in the -Z direction
		forward_x = DEFAULT_FORWARD_X = 0;
		forward_y = DEFAULT_FORWARD_Y = 0;
		forward_z = DEFAULT_FORWARD_Z = -1;
		...
		//By default, +X is "right"
		right_x = DEFAULT_RIGHT_X = 1;
		right_y = DEFAULT_RIGHT_Y = 0;
	...
	void updateCamera()
	{
		...
		convertAxisAngleToQuaternion(degreeToRadians(pitch),1.0f,0,0,pitch_qw,pitch_qx,pitch_qy,pitch_qz);//Build a quaternion for pitching around X axis
		float yaw_qw, yaw_qx, yaw_qy, yaw_qz = 0.0f;
		convertAxisAngleToQuaternion(degreeToRadians(yaw),0,1.0f,0,yaw_qw,yaw_qx,yaw_qy,yaw_qz);//Build a quaternion for yawing camera around Y axis
		float roll_qw, roll_qx, roll_qy, roll_qz = 0.0f;
		convertAxisAngleToQuaternion(degreeToRadians(roll),0,0,1.0f,roll_qw,roll_qx,roll_qy,roll_qz);//Build a quaternion for rolling camera around Z axis
	...
	void keyInput(unsigned char key)
	{
		switch (key)
		{
			case 'a': turnLeft(lookSpeed);		break;
			case 'A': turnRight(lookSpeed);		break;
			case 'T': lookUp(lookSpeed);		break;
			case 't': lookDown(lookSpeed);		break;
			case 'c': rollLeft(lookSpeed);		break;
			case 'C': rollRight(lookSpeed);		break;

			case 'I': moveForward(moveSpeed);	break;
			case 'i': moveBackward(moveSpeed);	break;
		 }
	}
	...
	void specialKeys(int key)
	{
		switch(key) 
		{
			case GLUT_KEY_LEFT:	panLeft(moveSpeed);		break;
			case GLUT_KEY_RIGHT:panRight(moveSpeed);	break;
			case GLUT_KEY_UP:	panUp(moveSpeed);		break;
			case GLUT_KEY_DOWN: panDown(moveSpeed);		break;
		}
	}
};
